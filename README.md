...
**SDK source:** [`client.go`](client.go)
**Runnable examples:** [`examples/`](examples/)
**E2E tests:** [`test/`](test/) ([setup instructions](test/README.md))
...
export TO_NUMBER="+1800***1234"
export FROM_NUMBER="+1555***4567"
go run ./outbound/

# 4. Listen for inbound calls on a DID
export DID="1800***1234"
go run ./inbound/
```
...
See [`examples/outbound/main.go`](examples/outbound/main.go) for a complete, runnable version with event logging.
...
| `authToken` | Auth token from CM config (e.g. `Basic YOUR_TOKEN`) |
...
```go
client := telephony.NewClient(wsURL, "Basic mytoken", "client-1", "YOUR_APPID")
```
...
```go
// Transfer the call
err := client.Transfer(ctx, callID, "+1800***4567", "")

// Transfer a specific leg
err := client.Transfer(ctx, callID, "+1800***4567", "B")
```
...
```go
client.SetSubscribeNumbers([]string{"+1800***1234", "+1800***9876"})
...
err := client.Subscribe(ctx, []string{"+1800***1234"})
```
...
result, err := client.Dial(ctx, telephony.DialParams{
    ...
    Token:   "agora-rtc-token",
    Region:  "AREA_CODE_NA",
    AppID:   "253***************************e537",    // required in MULTI mode
})
...
Runnable examples are in the [`examples/`](examples/) directory. Each is a standalone `main.go` with env var configuration.
...
Dialing +1800***1234 from +1555***4567 ...
...
| `AUTH_TOKEN` | yes | — | Auth token (e.g. `Basic YOUR_TOKEN`) |
| `APPID` | yes | — | Agora App ID |
| `TO_NUMBER` | outbound | — | Destination phone number (E.164 format) |
| `FROM_NUMBER` | outbound | `+1555***4567` | Caller ID phone number |
